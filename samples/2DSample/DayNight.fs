...
let getSunPosition (time: float32) (center: Vector2) (radius: Vector2) : Vector2 =
...
let getMoonPosition (time: float32) (center: Vector2) (radius: Vector2) : Vector2 =
...
let getSunColor (time: float32) : Color =
    if time < 5.0f || time > 19.0f then Color.Transparent // Off
    elif time < 7.0f then lerpColor Color.Transparent Color.Orange ((time - 5.0f) / 2.0f)
    elif time < 17.0f then Color.LightYellow
    elif time < 19.0f then lerpColor Color.LightYellow Color.OrangeRed ((time - 17.0f) / 2.0f)
    else Color.Transparent

let getMoonColor (time: float32) : Color =
    if time > 5.0f && time < 19.0f then Color.Transparent // Invisible during day
