...
let tests =
  testList "Graphics3D" [
    testList "CameraState" [
      testCase "billboardBasis Spherical uses camera basis"
      <| fun _ ->
      ...
      <| fun _ ->
        ...
        let struct (right, up) =
          CameraState.billboardBasis (Cylindrical Vector3.Up) camInfo pos
    ...
    testList "FrameOrchestration" [
      testCase "runCommandStream partitions opaque and transparent"
      <| fun _ ->
        ...
        buffer.Add(
          (),
          DrawMesh(Opaque, model, transform, ValueNone, ValueNone, ValueNone)
        )
      ...
      testCase "runCommandStream flushes on barriers"
      <| fun _ ->
        let buffer = RenderBuffer<RenderCmd3D>()
        let model = Unchecked.defaultof<Model>
        ...
        Expect.equal
          flushes
          2
          "Should have flushed twice (once at SetCamera, once at end)"

      testCase "runCommandStream calculates distance for sorting"
      <| fun _ ->
        let buffer = RenderBuffer<RenderCmd3D>()
        let model = Unchecked.defaultof<Model>
        ...
        buffer.Add(
          ...
          DrawMesh(
            Opaque,
            model,
            Matrix.CreateTranslation(0f, 0f, 5f),
            ValueNone,
            ValueNone,
            ValueNone
          )
        ...
        state.View <-
          Matrix.CreateLookAt(Vector3(0f, 0f, 10f), Vector3.Zero, Vector3.Up)
        ...
        Expect.floatClose
          ...
          (float d1)
          100.0
          "First mesh distance squared should be 100"

        Expect.floatClose
          Accuracy.medium
          (float d2)
          25.0
          "Second mesh distance squared should be 25"
    ]
    ...
    testList "Draw3D Builders" [
      testCase "mesh builder creates opaque command with defaults"
      <| fun _ ->
        let model = Unchecked.defaultof<Model>
        let transform = Matrix.Identity
      ...
      testCase "meshTransparent builder creates transparent command"
      <| fun _ ->
        let model = Unchecked.defaultof<Model>
        let transform = Matrix.Identity
      ...
      testCase "withColor modifies builder"
      <| fun _ ->
        let model = Unchecked.defaultof<Model>

        let cmd =
          Draw3D.mesh model Matrix.Identity |> Draw3D.withColor Color.Red

        Expect.equal cmd.Color (ValueSome Color.Red) "Color should be updated"

      testCase "quad3D creates correct geometry"
      <| fun _ ->
