...
module LayoutHelpers =
  ...
  /// </summary>
  let inline setLocal (lx: int) (ly: int) (content: 'T) (section: GridSection2D<'T>) : unit =
    let gx = section.OffsetX + lx
    let gy = section.OffsetY + ly
    if gx >= 0 && gx < section.BackingGrid.Width && gy >= 0 && gy < section.BackingGrid.Height then
...
module Layout =
  ...
  let inline run ([<InlineIfLambda>] f: GridSection2D<'T> -> GridSection2D<'T>) (grid: CellGrid2D<'T>) : CellGrid2D<'T> =
  ...
  /// </summary>
  let fill x y width height content (section: GridSection2D<'T>) : GridSection2D<'T> =
  ...
  /// </summary>
  let border x y width height content (section: GridSection2D<'T>) : GridSection2D<'T> =
  ...
  /// </summary>
  let inline generate x y width height ([<InlineIfLambda>] generator: int -> int -> 'T) (section: GridSection2D<'T>) : GridSection2D<'T> =
    for fx in x .. x + width - 1 do
      for fy in y .. y + height - 1 do
  ...
  /// </summary>
  let inline iter x y width height ([<InlineIfLambda>] action: int -> int -> 'T voption -> unit) (section: GridSection2D<'T>) : GridSection2D<'T> =
    for fx in x .. x + width - 1 do
      for fy in y .. y + height - 1 do
  ...
  /// </summary>
  let inline map x y width height ([<InlineIfLambda>] mapping: 'T -> 'T) (section: GridSection2D<'T>) : GridSection2D<'T> =
    for fx in x .. x + width - 1 do
      for fy in y .. y + height - 1 do
        let gx = section.OffsetX + fx
        let gy = section.OffsetY + fy
  ...
  /// </summary>
  let repeatX x y count content (section: GridSection2D<'T>) : GridSection2D<'T> =
  ...
  /// </summary>
  let repeatY x y count content (section: GridSection2D<'T>) : GridSection2D<'T> =
    for i in 0 .. count - 1 do
  ...
  /// </summary>
  let line x1 y1 x2 y2 content (section: GridSection2D<'T>) : GridSection2D<'T> =
    let dx = abs (x2 - x1)
    let dy = -abs (y2 - y1)
  ...
  /// </summary>
  let circle cx cy radius filled content (section: GridSection2D<'T>) : GridSection2D<'T> =
  ...
  /// </summary>
  let polygon (points: struct (int * int)[]) filled content (section: GridSection2D<'T>) : GridSection2D<'T> =
  ...
  /// </summary>
  let scatter count seed content (section: GridSection2D<'T>) : GridSection2D<'T> =
  ...
  /// </summary>
  let replace oldContent newContent (section: GridSection2D<'T>) : GridSection2D<'T> =
    for x in 0 .. section.Width - 1 do
      for y in 0 .. section.Height - 1 do
